...
impl BenchmarkResult {
    fn new(operation: String, tool: String, file_count: usize, total_size_mb: f64, time_ms: u128) -> Self {
...
fn create_test_files(dir: &Path) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    for i in 0..100 {
        let content = format!("This is test file {} with some repeated content. {}", i, "A".repeat(800));
...
fn benchmark_rusty_create(test_dir: &Path, archive_path: &Path) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
    ...
    if !output.status.success() {
        return Err(format!("Rusty create failed: {}", String::from_utf8_lossy(&output.stderr)).into());
    }
...
fn benchmark_rusty_extract(archive_path: &Path, extract_dir: &Path, file_count: usize, total_size_mb: f64) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
    ...
    if !output.status.success() {
        return Err(format!("Rusty extract failed: {}", String::from_utf8_lossy(&output.stderr)).into());
    }
...
fn benchmark_system_zip_create(test_dir: &Path, archive_path: &Path, file_count: usize, total_size_mb: f64) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
    let start = Instant::now();
    ...
    if !output.status.success() {
        return Err(format!("System zip failed: {}", String::from_utf8_lossy(&output.stderr)).into());
    }
...
fn benchmark_system_zip_extract(archive_path: &Path, extract_dir: &Path, file_count: usize, total_size_mb: f64) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
    fs::create_dir_all(extract_dir)?;
    ...
    if !output.status.success() {
        return Err(format!("System unzip failed: {}", String::from_utf8_lossy(&output.stderr)).into());
    }
...
fn run_benchmarks() -> Result<Vec<BenchmarkResult>, Box<dyn std::error::Error>> {
    ...
    println!("  Create: {:.0}ms ({:.2} MB/s)", rusty_create_result.time_ms, rusty_create_result.throughput_mbps);
    results.push(rusty_create_result);
    
    let rusty_extract_dir = temp_dir.path().join("rusty_extract");
    let rusty_extract_result = benchmark_rusty_extract(&rusty_archive, &rusty_extract_dir, file_count, total_size_mb)?;
    println!("  Extract: {:.0}ms ({:.2} MB/s)", rusty_extract_result.time_ms, rusty_extract_result.throughput_mbps);
    ...
    if Command::new("zip").arg("--version").output().is_ok() {
        ...
        if let Ok(zip_create_result) = benchmark_system_zip_create(&test_dir, &zip_archive, file_count, total_size_mb) {
            println!("  Create: {:.0}ms ({:.2} MB/s)", zip_create_result.time_ms, zip_create_result.throughput_mbps);
            results.push(zip_create_result);
            
            let zip_extract_dir = temp_dir.path().join("zip_extract");
            if let Ok(zip_extract_result) = benchmark_system_zip_extract(&zip_archive, &zip_extract_dir, file_count, total_size_mb) {
                println!("  Extract: {:.0}ms ({:.2} MB/s)", zip_extract_result.time_ms, zip_extract_result.throughput_mbps);
...
fn print_summary(results: &[BenchmarkResult]) {
    ...
    for result in results {
        ...
        println!("{:<12} {:<12} {:<8} {:<10.2} {:<8} {:<10.2} {:<10}", 
                 result.tool, result.operation, result.file_count, 
                 result.total_size_mb, result.time_ms, result.throughput_mbps, ratio_str);
    }
...
fn performance_benchmark() {
    match run_benchmarks() {
        Ok(results) => {
            ...
            let rusty_create = results.iter().find(|r| r.tool == "rusty" && r.operation == "create");
            let rusty_extract = results.iter().find(|r| r.tool == "rusty" && r.operation == "extract");
            
            if let Some(create_result) = rusty_create {
                assert!(create_result.throughput_mbps > 1.0, "Create throughput should be > 1 MB/s");
            ...
            if let Some(extract_result) = rusty_extract {
                assert!(extract_result.throughput_mbps > 5.0, "Extract throughput should be > 5 MB/s");
            }
        }
        Err(e) => {
