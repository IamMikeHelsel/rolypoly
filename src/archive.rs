...
impl ArchiveManager {
    ...
    pub fn create_archive<P: AsRef<Path>>(&self, archive_path: P, files: &[P]) -> Result<()> {
        ...
        for file_path in files {
            ...
            if path.is_file() {
                ...
                total_files += WalkDir::new(path).into_iter().filter_map(|e| e.ok()).filter(|e| e.path().is_file()).count();
            }
        }
    ...
    pub fn extract_archive<P: AsRef<Path>>(&self, archive_path: P, output_dir: P) -> Result<()> {
        ...
        println!("Extracting archive: {} to {}", archive_path.as_ref().display(), output_dir.as_ref().display());
        let pb = ProgressBar::new(archive.len() as u64);
        pb.set_style(ProgressStyle::default_bar()
            .template("[{elapsed_precise}] {bar:40.cyan/blue} {pos:>7}/{len:7} {msg}")
            .unwrap()
