...
class RolyPolyCli {
  ...
  Future<ProcessResult> create(
    ...
    if (kIsWeb)
      throw UnsupportedError('Process execution is unavailable on web');
    final args = ['create', archive, ...files, if (json) '--json'];
    return Process.run(binary, args);
  }

  Future<ProcessResult> extract(
    String archive,
    String outDir, {
    bool json = false,
  }) {
  ...
  Stream<Map<String, dynamic>> streamExtract(
    String archive,
    String outDir,
  ) async* {
    if (kIsWeb)
      throw UnsupportedError('Streaming process is unavailable on web');
  ...
  Stream<Map<String, dynamic>> streamCreate(
    String archive,
    List<String> files,
  ) async* {
    if (kIsWeb)
