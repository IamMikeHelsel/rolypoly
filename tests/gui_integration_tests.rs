...
async fn test_full_archive_workflow_gui() -> Result<()> {
    ...
    // 1. Create archive
    let archive_result = test_helpers::create_test_archive(&temp_dir, &[("test.txt", "GUI integration test")]);
...
async fn test_large_file_handling() -> Result<()> {
    ...
    let archive_path = test_helpers::create_test_archive(&temp_dir, &[("large.txt", &large_content)]);
