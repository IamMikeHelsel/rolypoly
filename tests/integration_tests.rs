...
fn test_end_to_end_archive_workflow() -> Result<()> {
    ...
    assert!(output.status.success(), "Archive creation failed: {}", String::from_utf8_lossy(&output.stderr));
    ...
    assert!(output.status.success(), "Stats command failed: {}", String::from_utf8_lossy(&output.stderr));
    ...
    assert!(output.status.success(), "Extraction failed: {}", String::from_utf8_lossy(&output.stderr));
    ...
    assert_eq!(fs::read_to_string(extract_dir.join("subdir/another.txt"))?, "Another nested file");
    ...
    assert!(output.status.success(), "Hash command failed: {}", String::from_utf8_lossy(&output.stderr));
...
fn test_error_handling_scenarios() -> Result<()> {
    ...
    let output = run_rusty_command(&[
        "extract",
        work_dir.join("nonexistent.zip").to_str().unwrap(),
    ])?;
...
fn test_large_file_handling() -> Result<()> {
    ...
    assert!(output.status.success(), "Large file archiving failed: {}", String::from_utf8_lossy(&output.stderr));
    ...
    assert!(output.status.success(), "Large file validation failed: {}", String::from_utf8_lossy(&output.stderr));
    ...
    assert!(output.status.success(), "Large file extraction failed: {}", String::from_utf8_lossy(&output.stderr));
...
fn test_special_characters_and_unicode() -> Result<()> {
    ...
    assert!(output.status.success(), "Unicode file archiving failed: {}", String::from_utf8_lossy(&output.stderr));
    ...
    assert!(output.status.success(), "Unicode file extraction failed: {}", String::from_utf8_lossy(&output.stderr));
    
    // Verify content
    for (filename, expected_content) in &files {
        let extracted_content = fs::read_to_string(extract_dir.join(filename))?;
        assert_eq!(&extracted_content, expected_content, "Unicode content mismatch for {}", filename);
    }
...
fn test_compression_effectiveness() -> Result<()> {
    ...
    assert!(output.status.success(), "Compression test archiving failed: {}", String::from_utf8_lossy(&output.stderr));
    ...
    assert!(output.status.success(), "Compression stats failed: {}", String::from_utf8_lossy(&output.stderr));
