...
fn stress_test_large_archive() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Compression: {:.1}% ({:.2} MB -> {:.2} MB)", 
             compression_ratio * 100.0, total_size_mb, archive_size_mb);
    ...
    assert!(create_throughput > 5.0, "Create throughput should be > 5 MB/s, got {:.2}", create_throughput);
    assert!(extract_throughput > 20.0, "Extract throughput should be > 20 MB/s, got {:.2}", extract_throughput);
    assert!(compression_ratio < 0.8, "Should achieve reasonable compression, got {:.1}%", compression_ratio * 100.0);
