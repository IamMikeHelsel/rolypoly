...
async fn test_concurrent_gui_operations() -> Result<()> {
    ...
    let t1 = tokio::spawn(async move {
        test_helpers::run_command_async(&["create", &archive1_s, &file1_s]).await
    });

    let t2 = tokio::spawn(async move {
        test_helpers::run_command_async(&["create", &archive2_s, &file2_s]).await
    });
