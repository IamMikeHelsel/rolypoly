...
async fn test_health_check_component() -> Result<()> {
    ...
    let output = test_helpers::run_binary_command(&["--version"]);
...
async fn test_create_archive_error_handling() -> Result<()> {
    ...
    let output = test_helpers::run_binary_command(&["create", "archive.zip", "non_existent_file.txt"]);
    assert!(!output.status.success());
    Ok(())
}
...
async fn test_list_archive_error_handling() -> Result<()> {
    ...
    let output = test_helpers::run_binary_command(&["list", "non_existent_archive.zip"]);
    assert!(!output.status.success());
    Ok(())
}
...
async fn test_validate_archive_error_handling() -> Result<()> {
    ...
    let bad_file_str = bad_file.to_str().unwrap();
    let output = test_helpers::run_binary_command(&["validate", bad_file_str]);
...
async fn test_calculate_file_hash_error_handling() -> Result<()> {
    // Test with non-existent file
    let output = test_helpers::run_binary_command(&["hash", "non_existent_file.txt"]);
    assert!(!output.status.success());
    Ok(())
}
...
async fn test_fun_error_messages() -> Result<()> {
    ...
    let output = test_helpers::run_binary_command(&["create"]); // Missing args
    assert!(!output.status.success());
...
async fn test_fun_success_messages() -> Result<()> {
    ...
    let output = test_helpers::run_binary_command(&["validate", archive_path_str]);
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
...
async fn test_concurrent_gui_operations() -> Result<()> {
    ...
    let t1 = tokio::spawn(async move {
        test_helpers::run_binary_command(&["create", &archive1_s, &file1_s])
    });

    let t2 = tokio::spawn(async move {
        test_helpers::run_binary_command(&["create", &archive2_s, &file2_s])
    });
